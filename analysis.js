...
async function calculateUserTransactions(account, storage, user_value, device_list) {
  ...
  // Serie is used to get only for this user.
  ...
  let [last_total_transactions] = await storage.getData({ variable: 'last_transactions', qty: 1, serie: user_value });
  ...
  await storage.sendData([
    { variable: 'last_transactions', value: total_transactions, serie: user_value },
    { variable: 'transactions_result', value: result, serie: user_value },
    { variable: 'user', value: user_value, serie: user_value },
  ]);
}
...
async function myAnalysis(context) {
  ...
  // ]
  const device_list = await account.devices.list({ page: 1, fields: ['id', 'name', 'bucket', 'tags'], filter: { tags: [{ key: tag_to_search }] }, amount: 10000 });
